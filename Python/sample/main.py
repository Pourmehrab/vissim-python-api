...
print
'Name of Link(', Link_number, '):', Name_of_Link
...
print
'Actual state of SignalHead(%d) is: %s' % (SH_number, State_of_SH)
...
for cnt_Veh in range(len(All_Vehicles)):
    ...
    print
    '%s  |  %s  |  %.2f  |  %.2f  |  %s' % (veh_number, veh_type, veh_speed, veh_position, veh_linklane)
...
new_Vehicle = Vissim.Net.Vehicles.AddVehicleAtLinkPosition(vehicle_type, link, lane, xcoordinate, desired_speed,
                                                           interaction)
...
for cnt_S in range(number_of_runs):
    print
    '%s | %d | %d ' % (List_Sim_Runs[cnt_S][0], List_Sim_Runs[cnt_S][1], List_Sim_Runs[cnt_S][2])
...
print
'Data Collection #', DC_measurement_number, ': Average values of all Simulations runs of 1st time intervall of all vehicle classes:'
print
...
print
'Average maximum Queue length of all simulations and time intervals of Queue Counter #%d: %.2f' % (QC_number, maxQ)
