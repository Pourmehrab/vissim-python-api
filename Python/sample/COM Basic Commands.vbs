...
' C:\Program Files\PTV Vision\PTV Vissim 8\Doc\Eng\Vissim 8 - COM Intro.pdf
'
...
Sub Run3Simulations()
  ...
  ' Check first if too many simulation runs were executed already:
  if Vissim.Net.SimulationRuns.Count > 2 then
    Dim warningString
    warningString = "More than 2 simulation runs have been executed already." + vbcrlf _
                + "This may lead to unexpected results or errors when executing subsequent functions." + vbcrlf _
                + "We recommend to delete all simulation runs before continuing:" + vbcrlf _
                + "While this message is still open, go to the 'Simulation runs' list" + vbcrlf _
                + "(which is tabbed with the scripts list) and delete all lines." + vbcrlf + vbcrlf _
                + "Press 'Yes' to continue with running the simulation 3 times." + vbcrlf 
    Dim result
    result = MsgBox (warningString, vbExclamation+vbYesNo, "Warning")
...
Sub SaveNetworkAndLayout()
  Dim path
  Dim filename
  path = "C:\Users\Public\Documents\PTV Vision\PTV Vissim 8\Examples Training\COM\Basic Commands\"
